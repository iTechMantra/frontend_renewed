// src/components/PharmacySearch.jsx
import { useState, useEffect } from 'react';
import { getMedicines } from '../services/storageService';
import { translate } from '../services/translationService';

export default function PharmacySearch({ onAddToCart }) {
              ...
              >
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className="flex-1 px-3 py-2 bg-yellow-100 text-yellow-700 text-sm rounded-md hover:bg-yellow-200 transition-colors"
                  >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="font-medium text-gray-900 mb-2">{translate('Name')}</h4>
                  <p className="text-gray-700">{selectedMedicine.name}</p>
                </div>

                <div>
                  <h4 className="font-medium text-gray-900 mb-2">{translate('Company')}</h4>
                  <p className="text-gray-700">{selectedMedicine.company}</p>
                </div>

                <div>
                  <h4 className="font-medium text-gray-900 mb-2">{translate('Dosage')}</h4>
                  <p className="text-gray-700">{selectedMedicine.dosage}</p>
                </div>

                <div>
                  <h4 className="font-medium text-gray-900 mb-2">{translate('Category')}</h4>
                  <p className="text-gray-700">{selectedMedicine.category}</p>
                </div>

                <div>
                  <h4 className="font-medium text-gray-900 mb-2">{translate('Price')}</h4>
                  <p className="text-2xl font-bold text-green-600">₹{selectedMedicine.price}</p>
                </div>

                {selectedMedicine.description && (
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">{translate('Description')}</h4>
                ...
                <div className="flex space-x-3 pt-4">
                  <button
                    ...
                    className="flex-1 bg-yellow-600 text-white py-2 px-4 rounded-md hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition-colors"
                  >
                    {translate('Add to Cart')}
                  </button>
